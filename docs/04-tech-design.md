...
## 4.0 試作で実際に採った構成（この文書との差分）

本設計書は Vite + React + Tailwind を前提に書いたが、**リポジトリにある試作は
ビルド不要の素の ES Modules + DOM 直操作**で実装してある。理由は3つ。

1. 「URL を開いたら即プレイ」という企画の前提（01.4）に、ビルド工程がない方が素直に合う。
2. `src/core` を `.js` の ES Modules にしておくと、**ブラウザと Node の両方から同じファイルを
   そのまま import できる**。シミュレータ（4.7）をブラウザ用とは別に用意しなくて済む。
3. 盤面は 20 マスの静的グリッドで、状態も 1 スピンに 1 回しか変わらない。
   仮想 DOM の差分計算が効く場面がない。

**4.1 以降の設計方針（core の純粋性、プリミティブ6種、シード付き乱数、
フェーズ制の解決順、シミュレータ）はすべてそのまま適用している。**
変えたのは描画層の道具立てだけ。規模が増えて描画層がつらくなったら、
core に手を入れずに React へ載せ替えられる。

型については、TypeScript ではなく JSDoc コメントで補っている。
本実装に進む際は TS 化を推奨する（4.3 の型定義はそのまま使える）。
