...
export const sfx = {
   ...
   * ノイズの粒＋square波という、音程ループを依頼される前からある素朴な
   * 質感（fc4bec2）をそのままベースにしている ── ここの音色自体は変えず、
   * その上に着地順の音程変化だけを足す。
   *
   * 着地順（pitchStep）に応じて square 波の音程が少しずつ**上がって
   * いく**。ループ（% 12で折り返す）ではなく、上がりきったら
   * PITCH_STEP_MAX で頭打ちにして、そこから先は同じ高さで鳴り続ける
   * だけ ── コイン枚数に上限がないため、際限なく上げ続けると聞き取れない
   * 高音域に突き抜けてしまう。ノイズの粒（クリック本体）は音程を運ばず、
   * ゆらぎだけを掛けたまま据え置く。ABSORB_STAGGER_MS（32ms）の等間隔で
   * 連続して着地するので、この上昇が「チッ、チチッ…」という自然な
   * 払い出し感になる。
   */
  coinLand(pitchStep = 0) {
    const step = Math.min(pitchStep, PITCH_STEP_MAX);
    const wobble = 1 + (Math.random() - 0.5) * 0.06;
    const freq = 1800 * Math.pow(2, (step * 1.4) / 12) * wobble;
    noise({ dur: 0.03, gain: 0.22, from: 6500 * wobble, to: 3000 * wobble, q: 5 });
    tone({ freq, type: 'square', dur: 0.018, gain: 0.09 });
  },
