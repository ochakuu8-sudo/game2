...
export const sfx = {
   ...
   * 打音そのものの質感（ノイズ主体の短いクリック）は変えないが、
   * 着地順（pitchStep）で音程を一段ずつ上げていき、12段でループする
   * ── 「コインが吸われる音の音程をループさせる仕様」を追加してほしい
   * という要望を受けたもの。ABSORB_STAGGER_MS（32ms）の等間隔で連続して
   * 着地するため、この上昇ループが「ジャラララ…」の中に軽い音階の
   * うねりとして重なって聞こえる。際限なく上げ続けると聞き取れない
   * 高音域に突き抜けてしまうので、12段（`% 12`）でループさせる。
   */
  coinLand(pitchStep = 0) {
    const step = pitchStep % 12;
    const freq = note(16 + step * 1.4);
    const wobble = 1 + (Math.random() - 0.5) * 0.06;
    noise({ dur: 0.03, gain: 0.22, from: 6500 * wobble, to: 3000 * wobble, q: 5 });
    tone({ freq: freq * wobble, type: 'square', dur: 0.018, gain: 0.09 });
  },
